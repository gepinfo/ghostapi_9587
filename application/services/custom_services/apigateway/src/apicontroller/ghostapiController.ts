...
export class ghostapiController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/ghostapi/:id', this.GpDelete);
this.router.get('/ghostapi/get/search', this.GpSearch);
this.router.put('/ghostapi/get/update', this.GpSearchForUpdate);
this.router.put('/ghostapi', this.GpUpdate);
this.router.get('/ghostapi/:id', this.GpGetNounById);
this.router.get('/ghostapi', this.GpGetAllValues);
this.router.post('/ghostapi', this.GpCreate);
    }

public GpDelete(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into ghostapiController.ts: GpDelete');
        new ApiAdapter().delete(Constant.GHOSTAPIURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdapter().get(Constant.GHOSTAPIURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/web' ? res.send(result) : res.send(null)
...
public GpSearchForUpdate(req: Request, res: Response) {
        ...
        new ApiAdapter().put(Constant.GHOSTAPIURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/web' ? res.send(result) : res.send(null)
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdapter().post(Constant.GHOSTAPIURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/web' ? res.send(result) : res.send(null)
